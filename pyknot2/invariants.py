...
def bar_natan(crossings, cuaps):
    ...
    print('cuaps are', cuaps)
    crossings = crossings.copy()
    # crossings[:, -1] *= -1  # Bar-Natan and van der Veen have the
    #                         # opposite sign convention?
    # cuaps[:, -1] *= -1

    feature_index = {}

    all_features = crossings[:, 0].tolist() + cuaps[:, 0].tolist()

    for index, label in enumerate(sorted(all_features)):
        feature_index[label] = index
    ...
    Q = sym.zeros(num_features)
    for crossing in crossings:
        if crossing[2] > 0:
            row = feature_index[crossing[0]]
            column = feature_index[crossing[1]]
            print('using {},{} from'.format(row, column), crossing)
            Q[row, column] = -1 * t**((sym1 if crossing[3] > 0 else -sym1) / 2)
            Q[column, column] = t**((sym1 if crossing[3] > 0 else -sym1) / 2)

    print('Q is:')
    print(repr(Q))

    W = sym.zeros(num_features)
    ...
    for crossing in unique_crossings:
        c *= t**(-1 * (1 if crossing[3] > 0 else -1))
    for cuap in cuaps:
        c *= t**(-1 * cuap[-1])

    print('c is:', c)

    B = sym.eye(num_features) - (t**symhalf - t**-symhalf) * W * Q
    B = sym.simplify(B)

    print('B is:')
    print(repr(B))

    G = sym.simplify(Q * B.adjugate())

    print('G is:')
    print(repr(G))

    H = sym.simplify(B.adjugate() * W)
    ...
    for j in range(2, num_features):
        for a in range(j):
            for b in range(j):
                inner_sum = 0
                for g in range(j+1, num_features):
                    inner_sum += G[g, b]
                ZG += G[j, a] * (sym1 / 2 * G[j, b] + inner_sum)
    ZG *= (t - 1/t)

    ZG = sym.expand(sym.simplify(ZG))
    ...
    for crossing in crossings:
        if not crossing[2] > 0:
            continue
        print('Doing crossing', crossing)
        sign = sym1 if crossing[3] > 0 else -sym1
        i = feature_index[crossing[0]]
        j = feature_index[crossing[1]]
        ZH += sign / 2 * ((1 - t**sign) * H[j, i])**2
        ZH -= sign / 2 * ((1 + t**sign) * H[j, j])**2
        ZH += sign * t**sign * (H[j, i] * H[i, j] + H[i, i] * H[j, j])
        ZH += t**sign * (1 - t) * H[j, i] * ((1 + sign)*H[j, j] +
                                             (1 - sign)*H[i, i])

    cuap_contribution = 0
    for cuap in cuaps:
        sign = sym.sympify(cuap[-1])
        print('cuap:', cuap)
        index = feature_index[cuap[0]]
        print('index:', index)
        cuap_contribution += sign * H[index, index]
    print('pre-cuap contribution:', sym.expand(sym.simplify(ZH)))
    print('pre-multiplier cuap contribution:', cuap_contribution)
    ZH += B.det() * cuap_contribution

    print('B.det():', B.det())
    print('cuap contribution:', sym.expand(sym.simplify(B.det() * cuap_contribution)))
